...
# SCRAM credential creation for KRaft mode (Kafka 4.x+)
# In KRaft mode, SCRAM credentials are managed via kafka-storage or kafka-configs
...
{
    echo "
    Usage: createScramCredentialsKRaft.sh [OPTIONS] none

    Create a user in test cluster (KRaft mode) with SCRAM 256 and 512 passwords,
    uses default credentials if none are passed
...
# KRaft mode uses --bootstrap-server instead of --zookeeper
docker exec \
 $(find_container_id) \
